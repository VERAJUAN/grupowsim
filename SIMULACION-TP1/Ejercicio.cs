...
namespace SIMULACION_TP1
{
    public partial class Ejercicio__B : System.Windows.Forms.Form
    {
        ...
        private void Simular(int desde = 0, int hasta = 0)
        {
            ...
            tablaVectorEstado.Columns[70].HeaderText = $"{pedidoACalcular} ENSAMBLES COMPLETADOS EN 1 HORA"; 
            tablaVectorEstado.Columns[71].HeaderText = $"PROBABILIDAD DE COMPLETAR {pedidoACalcular} O MAS ENSAMBLES";
            Random r = new Random();
                ...
                if (i == 0)
                {
                    ...
                    if(0 <= desde)
                    {
                        tablaVectorEstado.Rows.Add(vectorEstado.nroEvento.ToString(), Math.Round(vectorEstado.reloj, 3).ToString(), (Evento)vectorEstado.evento, vectorEstado.pedido == 0 ? "-" : vectorEstado.pedido.ToString(),
                            ...
                            Math.Round(vectorEstado.cantPromedioProdEnCola, 3).ToString(), Math.Round(vectorEstado.cantPromedioProdEnSistema, 3).ToString(), Math.Round(vectorEstado.porcOcupacionA1, 3).ToString(), Math.Round(vectorEstado.porcOcupacionA2, 3).ToString(), Math.Round(vectorEstado.porcOcupacionA3, 3).ToString(), Math.Round(vectorEstado.porcOcupacionA4, 3).ToString(), Math.Round(vectorEstado.porcOcupacionA5, 3).ToString(), Math.Round(vectorEstado.tiempoBloqueoColaA5, 3).ToString(), Math.Round(vectorEstado.propBloqueoSobreOcupacion, 3).ToString(), Math.Round(vectorEstado.tiempoBloqueoColaEncastreA3, 3).ToString(),
                            Math.Round(vectorEstado.tiempoBloqueoColaEncastreA3, 5).ToString(),
