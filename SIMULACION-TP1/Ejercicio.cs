        ...
        private void Simular(int desde = 0, int hasta = 0)
        {
            ...
            for (int i = 0; i < cantProyectos; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    if (0 <= desde)
                    {
                        tablaVectorEstado.Rows.Add(vectorEstado.nroEvento.ToString(), Math.Round(vectorEstado.reloj, 3).ToString(), (Evento)vectorEstado.evento, vectorEstado.pedido == 0 ? "-" : vectorEstado.pedido.ToString(),
                            ...
                            vectorEstado.ColaParaLavarToString(), 
                            ...
                            (Estado)vectorEstado.l2Estado, vectorEstado.l2Pedido == 0 ? "-" : vectorEstado.l2Pedido.ToString(), (i != 0 && vectorEstado.l2Pedido != vectorEstadoMasUno.l2Pedido && vectorEstado.l2Pedido != 0) ? Math.Round(numAleatorio4, 3).ToString() : "-", (i != 0 && vectorEstado.l2Pedido != vectorEstadoMasUno.l2Pedido && vectorEstado.l2Pedido != 0) ? Math.Round(vectorEstado.l2Tiempo, 3).ToString() : "-", (vectorEstado.l2ProxFin == 0 || vectorEstado.l2ProxFin == double.PositiveInfinity) ? "-" : Math.Round(vectorEstado.l2ProxFin, 3).ToString(),
                            vectorEstado.ColaParaSecarToString(),
                            (Estado)vectorEstado.sEstado, vectorEstado.sPedido == 0 ? "-" : vectorEstado.sPedido.ToString(), (i != 0 && vectorEstado.sPedido != vectorEstadoMasUno.sPedido && vectorEstado.sPedido != 0) ? Math.Round(vectorEstado.sTiempo, 3).ToString() : "-", (vectorEstado.sProxFin == 0 || vectorEstado.sProxFin == double.PositiveInfinity) ? "-" : Math.Round(vectorEstado.sProxFin, 3).ToString()
                            //vectorEstado.sColaaa.ToString(),
                            //(Estado)vectorEstado.encastreEstado, vectorEstado.encastrePedido == 0 ? "-" : vectorEstado.encastrePedido.ToString(), (i != 0 && vectorEstado.encastreTiempo != vectorEstadoMasUno.encastreTiempo && vectorEstado.encastreTiempo != 0) ? Math.Round(vectorEstado.encastreTiempo, 3).ToString() : "-", (vectorEstado.encastreProxFin == 0 || vectorEstado.encastreProxFin == double.PositiveInfinity) ? "-" : Math.Round(vectorEstado.encastreProxFin, 3).ToString(),vectorEstado.colaEncastrel1.ToString(), vectorEstado.colaEncastres.ToString(),
                            ...
                            //vectorEstado.maxCols.ToString(), vectorEstado.maxColaEncastre.ToString(), Math.Round(vectorEstado.tiempoPromColqa, 3).ToString(), Math.Round(vectorEstado.tiempoPromColaa, 3).ToString(), Math.Round(vectorEstado.tiempoPromColl1, 3).ToString(), Math.Round(vectorEstado.tiempoPromColl2, 3).ToString(), Math.Round(vectorEstado.tiempoPromCols, 3).ToString(), Math.Round(vectorEstado.tiempoPromColaEncastre, 3).ToString(),
                            //Math.Round(vectorEstado.cantPromedioProdEnCola, 3).ToString(), vectorEstado.cantPromedioProdEnSistema.ToString(), Math.Round(vectorEstado.porcOcupacionqa, 3).ToString(), Math.Round(vectorEstado.porcOcupacionaa, 3).ToString(), Math.Round(vectorEstado.porcOcupacionl1, 3).ToString(), Math.Round(vectorEstado.porcOcupacionl2, 3).ToString(), Math.Round(vectorEstado.porcOcupacions, 3).ToString(), Math.Round(vectorEstado.tiempoBloqueoColas, 3).ToString(), Math.Round(vectorEstado.propBloqueoSobreOcupacion, 3).ToString(), Math.Round(vectorEstado.tiempoBloqueoColaEncastrel1, 3).ToString(),
                ...
                else
                {
                    ...
                    //    lblCantMaxProductosS5.Text = vectorEstado.maxCols.ToString();
                    ...
                    //    lblPorcentajeOcupacionS5.Text = Math.Round(vectorEstado.porcOcupacions, 2).ToString() + "%";
