                ...
                if (i == 0)
                {
                    ...
                    if (0 <= desde)
                    {
                        tablaVectorEstado.Rows.Add(vectorEstado.nroEvento.ToString(), Math.Round(vectorEstado.reloj, 3).ToString(), (Evento)vectorEstado.evento, vectorEstado.pedido == 0 ? "-" : vectorEstado.pedido.ToString(),
                            ...
                            vectorEstado.ColaPaAAToString(), vectorEstado.ColaPaSToString(),
                            (Estado)vectorEstado.paEstado, vectorEstado.paPedido == 0 ? "-" : vectorEstado.paPedido.ToString(), (i != 0 && vectorEstado.paTiempo != vectorEstadoMasUno.paTiempo && vectorEstado.paTiempo != 0) ? Math.Round(vectorEstado.paTiempo, 3).ToString() : "-", (vectorEstado.paProxFin == 0 || vectorEstado.paProxFin == double.PositiveInfinity) ? "-" : Math.Round(vectorEstado.paProxFin, 3).ToString()
